...
const aspiration =
  /(?:경험|경력)\s*(?:이|은|는)?\s*없|(?:하지|보지|하지는|보지는)\s*않|사용해?\s*보지|해본\s*적[이 ]*없|하고\s*싶|배우고\s*싶|희망|예정|목표로|경험하고자|경험이\s*아닌|관심을\s*(?:갖게|가지게)|지원하게\s*되|\bno experience\b|\bnever\b|\bwant to\b|\bplan to\b/i;
const privateLine =
